...
**data preparation**
  ...
  ```{r}
mydata <- read.csv("customer_data", stringsAsFactors = TRUE)
...
bothsampled_training<-ovun.sample(visit~.,data=training,method="both",p=0.5,seed = 1)$data

table(bothsampled_training$visit)
prop.table(table(bothsampled_training$visit))
```
...
visit_weights <- information.gain(visit~., bothsampled_training)
...
* Let's plot the weights in descending order.
...
filtered_attributes <- cutoff.k(visit_weights, 11)
...
datamodelling <- bothsampled_training[filtered_attributes]#delete the non-related variable
...
datamodelling$target <- bothsampled_training$visit
...
# Find the best cost value among the list (0.5, 1.5) 
tune_out = tune(svm, target ~ ., data = datamodelling, kernel= "radial", scale = TRUE, probability=TRUE, ranges = list(cost=c(0.5, 1.5)))
...
model_RF <- randomForest(target ~ ., datamodelling)
...
#这个prob要变成tree
prob_DT <- predict(dectree, test, type = "prob")
prob_DT_prune <- predict(decTree_prune, test, type = "prob")
```
